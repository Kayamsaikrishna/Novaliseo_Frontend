...
export function extendMaterial(BaseMaterial, cfg) {
...
const CanvasWrapper = ({ children }) => (
  <Canvas dpr={[1, 2]} frameloop="always" className="w-full h-full relative" style={{ position: 'absolute', inset: 0 }}>
    {children}
  </Canvas>
);
...
export default function Beams({
  beamWidth = 2.5, beamHeight = 18, beamNumber = 15, lightColor = "#0ea5e9",
  speed = 2.5, noiseIntensity = 0, scale = 0.15, rotation = 43,
}) {
...
vec3 getNormal(vec3 pos) {
  ...
  gl_FragColor.rgb -= randomNoise / 15. * uNoiseIntensity;`,
      ...
      material: { fog: true },
      uniforms: {
        diffuse: new THREE.Color(...hexToNormalizedRGB("#020617")), // Dark slate bg
        time: { shared: true, mixed: true, linked: true, value: 0 },
        roughness: 0.1,
        metalness: 0.8, // Make it more metallic/liquid
  ...
  return (
    <CanvasWrapper>
      <group rotation={[0, 0, degToRad(rotation)]}>
        ...
        <DirLight color="#3b82f6" position={[5, -5, 5]} />
      </group>
      <ambientLight intensity={1.5} />
      {/* Remove attached background to allow transparency if desired, but here we match hero background */}
